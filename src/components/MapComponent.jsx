...
export default function MapComponent({ onMarkerClick, filterQuery, boroughFilter, panelOpen, cafes }) {
  ...
  const plotMarkers = (data) => {
    ...
    data.forEach((row) => {
      ...
      el.style.backgroundImage = 'url("/cafe-pin.png")';
  ...
  useEffect(() => {
    ...
    const boro = (boroughFilter || "").trim().toLowerCase();
    ...
    markersRef.current.forEach(({ marker, data }) => {
      ...
      const matchesBorough = !boro || (data.County || "").toLowerCase().includes(boro);

      if (matchesSearch && matchesBorough) {
        if (!marker._map) marker.addTo(mapRef.current);
        matches.push(data);
      } else {
        marker.remove();
